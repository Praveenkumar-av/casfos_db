  ...
  useEffect(() => {
    if (activeTab === "permanent") {
      axios.get(`${serverBaseUrl}/api/assets/permanent`)
        .then(response => setPermanentAssets(response.data))
        .catch(error => {
  ...
  useEffect(() => {
    if (activeTab === "consumable") {
      axios.get(`${serverBaseUrl}/api/assets/consumable`)
        .then(response => setConsumableAssets(response.data))
        .catch(error => {
    ...
    <div className="asset-updation">
      ...
      <section id="sidebar">
        ...
        <ul className="side-menu top">
          ...
          <li className="active"><a href={`/assetupdation?username=${encodeURIComponent(username)}`}><i className="bx bxs-package" /><span className="text">Asset Updation</span></a></li>
...
const componentStyles = {
  ...
  editButton: { padding: "8px 16px", backgroundColor: "#28a745", color: "#fff", border: "none", borderRadius: "5px", cursor: "pointer" },
